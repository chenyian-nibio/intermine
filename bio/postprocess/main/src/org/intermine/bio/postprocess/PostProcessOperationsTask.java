	...
	public void execute() {
		...
		try {
			if ("create-chromosome-locations-and-lengths".equals(operation)) {
				...
				InputStream classesToIgnoreStream = classLoader.getResourceAsStream(ignoreFileName);
				...
				(new PrecomputeTask())
						.precompute(false, getObjectStoreWriter().getObjectStore(), 0);
				...
				/*
				 * String configFileName = "objectstoresummary.config.properties"; InputStream
				 * configStream = classLoader.getResourceAsStream(configFileName); if (configStream
				 * == null) { throw new RuntimeException("can't find resource: " + configFileName);
				 * }
				 * 
				 * Properties properties = new Properties(); properties.load(configStream);
				 */
				...
				Map<String, List<FieldDescriptor>> classKeys = ClassKeyHelper.readKeys(
						os.getModel(), classKeyProperties);
				...
				CreateLocationOverlapIndex cloi = new CreateLocationOverlapIndex(
			...
			// chenyian:
				...
				nat.doAnalysis();
			// chenyian:
			} else if ("integrated-pathway-clustering".equals(operation)) {
				IntegratedPathwayClustering ipc = new IntegratedPathwayClustering(
						getObjectStoreWriter());
				ipc.doClustering();
