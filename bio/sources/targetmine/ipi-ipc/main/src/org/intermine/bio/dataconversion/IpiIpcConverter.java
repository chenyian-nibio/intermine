...
//	private static Logger m_oLogger = Logger.getLogger(IpiIpcConverter.class);
	...
	public IpiIpcConverter(ItemWriter writer, Model model) {
		super(writer, model);
		m_oWriter = writer;
	}
	...
	public void process(Reader reader) throws Exception {
		...
		while (oBr.ready()) {
			...
			String strDbName = a_strFields[0];
			String strDbId = a_strFields[1];
			String strInterProId = a_strFields[3];
			String strMemberDbId = a_strFields[4];
			String strFromPos = a_strFields[5];
			String strToPos = a_strFields[6];

			// SwissProt Only
			if (!"SP".equals(strDbName)) {
				continue;
			}
	...
	/**
	 * Get isoform number of argument strUniprotAcc. If strUniprotAcc is "A12345-1", return 1. If
	 * strUniprotAcc is not in isoform format, return 0.
	 * 
	 * @param strUniprotAcc
	 * @return
	 */
