 ...
 * @author chenyian
 */
public class PspReactionConverter extends BioFileConverter
{
	private static final Logger LOG = Logger.getLogger(PspReactionConverter.class);
	//
     ...
     */
    public PspReactionConverter(ItemWriter writer, Model model) {
...
//				LOG.info(String.format("%s->%s", kinaseGeneId, targetGeneId));
				...
				Item reaction = createReaction(kinaseGeneId, targetGeneId);
...
//								throw new RuntimeException(String.format("Inconsistence residue. Unable to correct the position, skip the entry. %s: %s-%d.", proteinId, residue, position)); 
						...
						reaction.addToCollection("modifications", getModification(accession, String.valueOf(position), residue));
					}
	...
	private Item createReaction(String geneId1, String geneId2) throws ObjectStoreException {
		String name = String.format("%s->%s", geneId1, geneId2);
		Item ret = reactionMap.get(name);
		if (ret == null) {
			ret = createItem("Reaction");
			...
			ret.addToCollection("types", getReactionType(PHOSPHORYLATION.toLowerCase()));
			
			reactionMap.put(name, ret);
		}
		return ret;
	}
