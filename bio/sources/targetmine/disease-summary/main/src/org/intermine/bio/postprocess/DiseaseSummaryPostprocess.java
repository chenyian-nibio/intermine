        ...
        while (iterator.hasNext()) {
			...
			try {
				...
				for (InterMineObject vaItem : snps) {
					...
					Set<InterMineObject> genomeWideAssociations = (Set<InterMineObject>) snpItem.getFieldValue("genomeWideAssociations");
					for (InterMineObject gwasItem : genomeWideAssociations) {
						InterMineObject publicationItem = (InterMineObject) gwasItem.getFieldValue("publication");
				...
				for (InterMineObject efot : gwasMap.keySet()) {
					...
					InterMineObject item = (InterMineObject) DynamicUtil.simpleCreateObject(
							model.getClassDescriptorByName("GeneticDisease").getType());
...
//					LOG.info("create object: " + String.format("%s > %s", symbol, diseaseName));
        ...
        while (iterator.hasNext()) {
        	...
        	try {
        		...
        		for (InterMineObject vaItem : snps) {
        			InterMineObject snpItem = (InterMineObject) vaItem.getFieldValue("snp");
        			Set<InterMineObject> alleles = (Set<InterMineObject>) snpItem.getFieldValue("alleles");
...
//        			LOG.info("create object: " + String.format("%s > %s", symbol, diseaseName));
	...
	private Iterator<?> getGwasGenes() throws ObjectStoreException {
		Query q = new Query();
		QueryClass qcGene = new QueryClass(os.getModel().getClassDescriptorByName("Gene").getType());
		QueryClass qcVariationAnnotation = new QueryClass(os.getModel().getClassDescriptorByName("VariationAnnotation").getType());
		QueryClass qcSnp = new QueryClass(os.getModel().getClassDescriptorByName("SNP").getType());
		QueryClass qcGwas = new QueryClass(os.getModel().getClassDescriptorByName("GenomeWideAssociation").getType());
		...
		cs.addConstraint(new ContainsConstraint(qcr1, ConstraintOp.CONTAINS, qcVariationAnnotation));
		...
		QueryCollectionReference qcr2 = new QueryCollectionReference(qcSnp, "genomeWideAssociations");
	...
	private Iterator<?> getClinvarGenes() throws ObjectStoreException {
		...
		QueryClass qcVariationAnnotation = new QueryClass(os.getModel().getClassDescriptorByName("VariationAnnotation").getType());
		QueryClass qcSnp = new QueryClass(os.getModel().getClassDescriptorByName("SNP").getType());
		QueryClass qcAllele = new QueryClass(os.getModel().getClassDescriptorByName("Allele").getType());
		QueryClass qcVariation = new QueryClass(os.getModel().getClassDescriptorByName("Variation").getType());
