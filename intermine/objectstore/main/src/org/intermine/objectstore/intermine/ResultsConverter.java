...
public final class ResultsConverter {
	...
	/**
	 * Method to convert from SQL results to InterMine object-based results. This method accepts an
	 * SQL ResultSet and a Query as an input. The ResultSet must contain a column named the same as
	 * the aliases of the elements in the SELECT list of the Query, each containing either the
	 * OBJECT column in the case of a business object, or the value of any other value. <br>
	 ...
	 *            the Query
	 * @param os
	 *            the ObjectStoreInterMineImpl with which to associate any new lazy objects
	 * @param c
	 *            a Connection with which to make extra requests
	 * @param sequence
	 *            an object representing the state of the database
	 * @param optimise
	 *            whether to use optimisation on path expression queries
	 * @param extra
	 *            object to record extra query execution time
	 * @param goFasterTables
	 *            a Set of PrecomputedTables that may help with extra queries
	 * @param goFasterCache
	 *            an OptimiserCache that may help with extra queries
	 * @return a List of ResultsRow objects
	 * @throws ObjectStoreException
	 *             if the ResultSet does not match the Query in any way, or if a SQL exception
	 *             occurs
	 */
			...
			while (sqlResults.next()) {
				...
				for (QuerySelectable node : q.getSelect()) {
					...
					if (node instanceof QueryClass) {
						...
						if (obj == null) {
							...
							if (noObjectColumns.contains(node)) {
								...
								if (os.getSchema().isFlatMode(((QueryClass) node).getType())) {
									obj = buildObject(sqlResults, alias, os, ((QueryClass) node)
											.getType(), noObjectClassColumns);
									if (idField != null) {
						...
						// chenyian: when only null value is found in a Date type column, converting
						// null to a Date object will cause a NullPointerException - 20111031
	 ...
	 *            the ObjectStore
	 * @param type
	 *            a Class matching the QueryClass that is this column
	 * @param noObjectClassColumns
	 *            a Set used internally
	...
	protected static Object buildObject(ResultSet sqlResults, String alias,
		...
		Set<Class<?>> classes = (Set) Collections.singleton(type);
		...
		Map<String, FieldDescriptor> fields = os.getModel().getFieldDescriptorsForClass(
				retval.getClass());
			...
			if (fd instanceof AttributeDescriptor) {
				...
				if ((value instanceof Long) && Date.class.equals(expectedType)) {
					value = new Date(((Long) value).longValue());
						...
						&& (Short.class.equals(expectedType) || Short.TYPE.equals(expectedType))) {
				...
				@SuppressWarnings("unchecked")
				Collection lazyColl = new ProxyCollection(os, (InterMineObject) retval, cd
						.getName(), cd.getReferencedClassDescriptor().getType());
				...
				// timeSpentSql += System.currentTimeMillis() - time3;
				@SuppressWarnings("unchecked")
				Class<? extends InterMineObject> refType = (Class) rd
	 ...
	 *            the Query
	 * @param qope
	 *            the QueryObjectPathExpression to fetch data for
	 * @param retval
	 *            the array of results that will be returned
	 * @param optimise
	 *            whether to optimise the query
	 * @param extra
	 *            object to record extra query execution time
	 * @param goFasterTables
	 *            a Set of PrecomputedTables that may help with extra queries
	...
	protected static void fetchObjectPathExpression(ObjectStoreInterMineImpl os, Connection c,
		...
		if (startingPoint == -1) {
			throw new ObjectStoreException("Path Expression " + qope + " needs QueryClass " + qc
		...
		List<ResultsRow<Object>> res = os.executeWithConnection(c, qopeQuery, 0, Integer.MAX_VALUE,
	 ...
	 *            the Query
	 * @param qcpe
	 *            the QueryCollectionPathExpression to fetch data for
	 * @param retval
	 *            the array of results that will be returned
	 * @param optimise
	 *            whether to optimise the query
	...
	protected static void fetchCollectionPathExpression(ObjectStoreInterMineImpl os, Connection c,
		...
		startingPoint = q.getSelect().indexOf(qc);
		if (startingPoint == -1) {
			throw new ObjectStoreException("Path Expression " + qcpe + " needs QueryClass " + qc
					+ " to be in the SELECT list");
		}
	 ...
	 *            an object representing the state of the database
	 * @param clazz
	 *            the class of the table to search in
	 * @param idsToFetch
	 *            a Collection of IDs to fetch
	 * @param extra
	 *            object to record extra query execution time
	...
	protected static Map<Integer, InterMineObject> fetchByIds(ObjectStoreInterMineImpl os,
		...
		try {
			...
			long startTime = System.currentTimeMillis();
			Iterator<ResultsRow<Object>> iter = os.executeWithConnection(c, q, 0,
					Integer.MAX_VALUE, false, false, sequence).iterator();
			extra.addTime(System.currentTimeMillis() - startTime);
			if (bttr != null) {
